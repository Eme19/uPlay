...
function ArtistDetail() {
  ...
  return (
    <div className="pt-ar  ">
      <div className="centered-content">
        <div className="Artist-cover ">
          {fetchedArtist.image && (
            <img
              className="art-img"
              src={fetchedArtist.image[0]}
              alt={fetchedArtist.name}
              style={{ maxWidth: "50%" }}
            />
          )}
        </div>

        <div className="  text-lg pr-20 pt-2 text-stone-200">
          {fetchedArtist.name}
        </div>
      </div>
      <ul>
        {tracks.map((track) => (
          <li key={track._id}>
            <div>
              <img
                src={track.image || "default_image_url"}
                alt={track.name}
      ...
      <div className="flex justify-start gap-10 pl-5 justify-normal pt-10">
        {albums.map((album) => (
                ...
                <div className="album-info">
                  <ul className="album-title">
                    <li
                      id="li-styl-h"
                      style={{ color: "#be185d" }}
                      className="align-baseline text-left"
                    >
                      {album.title}
                    </li>
                    <li id="li-styl">
                      <li
                        id="li-styl-h"
                        className="pt- text-left align-baseline text-zinc-300"
                      >
                        {new Date(album.updatedAt).getFullYear()}
                      </li>
                    </li>
                  </ul>
                </div>
